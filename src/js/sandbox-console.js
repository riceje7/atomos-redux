...
var Sandbox = {
	...
	Model: Backbone.Model.extend({
		...
		defaults: {
			history: [],
			iframe: false,
			// if true, run `eval` inside a sandboxed iframe
		...
		iframeSetup: function () {
			this.sandboxFrame = $('<iframe width="0" height="0"/>').css({
				visibility: 'hidden'
			}).appendTo('body')[0];
		...
		evaluate: function (command) {
			if (!command) {
				return false;
			} else if (!this.specialCommands(command)) {
				...
				try {
					item.result = this.get('iframe') ? this.iframeEval(command) : $.globalEval(command);
		...
		specialCommands: function (command) {
			if (command === ":clear") {
				...
				ig.game.player.commands.help();
			} else if (command.indexOf(":log") === 0) {
				return this.addHistory({
					command: ':log',
					result: command.substring(5)
				});
			} else if (command.indexOf(":answer") === 0) {
				ig.game.player.commands.answer(command.substring(8));
				return this.addHistory({
					command: ':answer',
					result: command.substring(8)
				});
		...
		keydown: function (e) {
			...
			if (e.which === 9) {
				...
				var value = this.textarea.val(),
					caret = this.getCaret(),
					parts = [
					value.slice(0, caret), value.slice(caret, value.length)];
